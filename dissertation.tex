...
This section displays the results of the data analysis described in SECTION. I
...
Figure~\ref{fig:suku_histogram} shows the results of the micro-timing estimation (SUBSECTION) for one of
...
maximum-likelihood estimated normal distributions (SUBSECTION). The plots for
...
Figure~\ref{fig:waltz_histogram} shows the results for the waltz dataset (SUBSECTION). The calculations
...
of the second beat can be clearly seen in the plot ($\mu=-0.0743,\sigma=0.0795$). The
one-sample \textit{t}-test confirms the micro-timing is significant ($t=-16.5,p=0.000$).
...
`Suku'. The tempo starts at around 135 bpm at the beginning of the piece and
...
\begin{figure}[ht]
    ...
    \caption{Tempo estimation for `Suku'. Grey: Smoothed estimated tempo for each recording. Blue: Mean estimated tempo averaged over each recording. Shaded: One standard deviation from the mean. Black: Curves fit to the data.}
...
previous work [Neuhoff2017] which studied experts' perception and judgement of
...
SUBSECTION discusses the results in more detail.
...
\begin{enumerate}
    \item Participants think samples with the data-derived micro-timing (the ``correct'' timing) fits the style best
    \item Participants with more musical experience are more likely to think the correct timing fits best than those with less experience
\end{enumerate}
...
data analysis results (SECTION) showed an SML pattern for the micro-timing in
...
Ethics approval was granted prior to recruitment (see APPENDIX for details on
the ethical and data protection procedures used).
...
Manjanin [Polak2010], and the full recording. For the waltz, these consisted of
...
each with different micro-timing (see SUBSECTION for details on the construction
of the stimuli). They were then asked to consider each stimulus and assess how
...
Following Neuhoff et al.\ [Neuhoff2017], the jembe stimuli consisted of a
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\linewidth}{
        ...
        \bottomrule
    \end{tabularx}
    \caption{A typical rhythmic pattern from the piece `Manjanin'. A letter at a metric location indicates a drum stroke. The letter represents the timbre: Tone, Slap, or Bass for the jembe, and open (O) or closed (X) for the dundun.}
    \label{table:manjanin_rhythm}
\end{table}

The micro-timing in jembe is at the pulse level, and the patterns are the same
on each beat. Table 4 in Neuhoff et al.\ contains data for the length of each
pulse as a proportion of the total beat length for five different timing
...
results of my data analysis in SECTION showed a SML pattern, so offsets for this
...
\begin{equation}
    \begin{split}
        ...
        \mathrm{offset}_i &= \mathrm{phase}_i - \mathrm{metricLocation}_i
    \end{split}
    \label{eq:proportion_to_offset}
\end{equation}

The probability distributions used the calculated or data-derived offsets as
their mean and used the same standard deviations as in the data analysis to try
to maintain the expressivity of the performance.
...
The micro-timing in Viennese waltz is at the beat level and features a
characteristically short first beat. Table 3 in Neuhoff et al. contains similar
...
Figure~\ref{fig:jembe_bar_chart_overall} shows a bar chart of the mean goodness-of-fit score for each timing
...
\begin{figure}[ht]
    ...
    \caption{Mean goodness-of-fit score for each timing pattern for jembe. Error bars show 95\% confidence interval.}
...
there being some suggestion that Hypothesis 1 is true for jembe, there is
...
significant ($t=1.05, p=0.152$). Therefore, despite there being some suggestion
that Hypothesis 2 is true for jembe, there is also insufficient evidence to
accept it.
...
\begin{figure}[ht]
    ...
    \caption{Mean goodness-of-fit score for each timing pattern for Viennese waltz. Error bars show 95\% confidence interval.}
...
the \textit{t}-test found to be significantly higher than any other pattern. When looking
...
than SLL, although the difference wasn't statistically significant. Therefore,
Hypothesis 1 is supported by the data for the waltz style.
...
non-musicians ($t=-2.05, p=0.027$). Therefore, Hypothesis 2 is supported by the
...
Non-musicians showed a preference (albeit statistically insignificant) for the
...
isochronous. This suggests that many non-musicians may have failed to pick up on
...
the joint-highest scoring pattern.
...
pulses making up the SML pattern occur rapidly. In fact, all three pulses are
...
music, as in Neuhoff et al.\ [Neuhoff2017].
...
describing it as ``realistic and beautiful''. Therefore, despite the
participants in the study struggling to identify the expected pattern, it is
clear the implementation can successfully produce realistic music.
...
Therefore a conversion back to MusicXML would likely have a different result to
the original music.

Figure~\ref{fig:musicxml_before} shows a short line of music written in Western music notation,
representing a MusicXML file. This was then converted to Sonic Pi code (see
APPENDIX for the code), then converted back to MusicXML. The final result is
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{\textwidth}
        ...
        \caption{After round-trip conversion}
        \label{fig:musicxml_after}
    \end{subfigure}
    \caption{A short line of music stored in a MusicXML file. Shown before and after a round-trip conversion to Sonic Pi and back to MusicXML.}
